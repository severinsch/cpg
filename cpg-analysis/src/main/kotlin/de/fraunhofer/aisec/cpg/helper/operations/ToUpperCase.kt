...
class ToUpperCase : Operation(2) {
            ...
            state.outgoingEdges =
                state.outgoingEdges
                    .map { edge ->
                        ...
                        return@map edge.copy(op = edge.op.uppercase())
                    }
                    .toSet()
        }
    }
    ...
    override fun charsetTransformation(cs: CharSet): CharSet {
        return when (cs) {
            is SetCharSet -> SetCharSet(cs.chars.flatMap { it.uppercase().asSequence() })
            is SigmaCharSet -> {
    ...
    override fun toString() = "toUpperCase"
}
